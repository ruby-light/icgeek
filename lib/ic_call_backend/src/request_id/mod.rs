...
use crate::sha256::{Sha256Algorithm, Sha256Hash};
...
impl RequestId {
    pub fn new(from: &Sha256Hash) -> RequestId {
        RequestId(*from)
    }
...
enum Hasher {
    ...
    Value(Sha256Algorithm),
}

impl Hasher {
    fn request_id() -> Hasher {
        Hasher::RequestId(Sha256Algorithm::default())
    }
    ...
    fn value() -> Hasher {
        Hasher::Value(Sha256Algorithm::default())
    }
}
